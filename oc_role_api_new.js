...
// @author       MrChurch[3654415], xentac[3354782], underko[3362751]
...
(function () {
    ...
    // Existing Hardcoded Restrictions (Fallback)
    let ocRoleInfluence = {
        "Pet Project": [
                ...
                onload: function (response) {
                    if (response.status === 200) {
                        try {
                            ...
                            console.log("[OCRoleRestrictionsAPI] Loaded Role Weights:", ocWeights);
                            // Weights loaded -> Force UI update to override any hardcoded defaults
                            refreshCrimes();
                        } catch (e) {
                            console.error("[OCRoleRestrictionsAPI] Error parsing weights:", e);
                        }
                    } else {
                        console.log("[OCRoleRestrictionsAPI] Failed to load weights, status:", response.status);
                    }
                },
                onerror: function (err) {
                    console.error("[OCRoleRestrictionsAPI] Network error fetching weights:", err);
                }
            });
        } catch (e) {
            console.error("[OCRoleRestrictionsAPI] GM_xmlhttpRequest failed:", e);
        }
    }

    // Calculate suggested lower bound based on weight using Piecewise Linear Interpolation
    // Agrees with community values (e.g., Looter@Wt7->42%, Looter4@Wt21->67%, Max@Wt50->75%)
    function getLowerFromWeight(weight) {
        let val;
        if (weight < 8) {
            // Low Impact Filler (0 to 8 weight -> 40 to 42%)
            val = 40 + (weight * 0.25);
        } else if (weight < 20) {
            // The Competence Jump (8 to 20 weight -> 60 to 67%)
            val = 60 + ((weight - 8) * 0.6);
        } else if (weight < 40) {
            // Linear Climb (20 to 40 weight -> 67 to 75%)
            val = 67 + ((weight - 20) * 0.4);
        } else {
            // Diminishing Returns (40+ weight -> 75%+)
            // Soft cap that creeps up very slowly
    ...
    function classifyOcRoleInfluence(ocName, roleName) {
        ...
        // Fallback to hardcoded logic if no weight found
    ...
    function getFactionId() {
        ...
        try {
            ...
            console.log("[OCRoleRestrictionsAPI] Couldn't extract faction id:", e);
        }
        ...
        try {
            GM_xmlhttpRequest({
                ...
                onload: async function (response) {
                    if (response.status != 200) {
                        console.error(
                            "[OCRoleRestrictionsAPI] Bad response fetching faction restrictions:",
                            response.status,
                        );
                        return cb();
                    }
                    ...
                    try {
                        ...
                        console.error(
                            "[OCRoleRestrictionsAPI] Failed to parse faction restrictions:",
                            error.message,
                        );
                    }
                    return cb();
            ...
            console.error(
                "[OCRoleRestrictionsAPI] Failed fetching faction restrictions:",
                error.message,
            );
        }
    }
    ...
    function processCrime(wrapper) {
        ...
        const cleanCrimeTitle = normalizeKey(crimeTitle); // For weight lookup
        const roles = [];

        const roleEls = wrapper.querySelectorAll(".title___UqFNy");
        roleEls.forEach((roleEl) => {
            const roleName = roleEl.textContent.trim();
            const cleanRoleName = normalizeKey(roleName); // For weight lookup
    ...
    if (factionId) {
        ...
        console.log(
            "[OCRoleRestrictionsAPI] Couldn't find faction id, going with defaults.",
        );
        cb();
    }
