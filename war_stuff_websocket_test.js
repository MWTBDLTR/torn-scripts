// ==UserScript==
// @name         Torn War Stuff Enhanced & Optimized (JSON)
...
(async function () {
  ...
  const TIME_BETWEEN_FRAMES = 250; // Faster refresh (4fps)
  ...
  console.log(
    `%c[TWSEO] Script Loaded (v5.1.1) | Debug: ${DEBUG} | Status Log: ${LOG_STATUS} | Sort Okay Score: ${SORT_OKAY_BY_SCORE}`,
    ...
    socket.onmessage = (event) => {
      ...
      messages.forEach(rawLine => {
        ...
        try {
          ...
          // Only log real errors, not "unexpected end of data" from empty lines
          console.error("[TWSEO] WS Parse Error on line:", line, e);
        }
      });
    };
  ...
  // NEW: Optimized JSON Processor
  function processWebSocketStatus(updateData) {
    // Handle batch updates (Torn sends arrays sometimes)
    ...
    updates.forEach(u => {
      ...
      // s.updateAt is the timestamp when the status ENDS (unix seconds)
      let newUntil = 0;
      if (!s.okay && s.updateAt) newUntil = s.updateAt;

      // Generic description from WS. API will overwrite with details (e.g. "Traveling to Mexico")
      const newDesc = newState + " (WS)";

      // Check for changes (State diff OR Timer diff > 5s)
      ...
      if (currentData.status.state !== newState || timerDiff > 5) {
        ...
        currentData.status.until = newUntil; // Exact end time
        currentData.status.updated = Date.now();
        currentData.status.freshOkay = !!s.okay; // Sink "Okay" to bottom
  ...
  async function update_status_single(faction_id) {
    try {
      ...
      for (const [k, v] of Object.entries(status.members)) {
        ...
        // If API says "Okay" but WS recently said "Hospital", keep WS.
        // If API says "Traveling to X" and WS says "Traveling", we allow update (to get details).
        const current = member_status.get(k);
        if (current) {
          const curState = current.status.state;
          const isWs = (current.status.description || "").includes("(WS)");
          // If recent WS Hospital/Jail/Traveling vs API Okay -> Skip API
          if (["Traveling", "Hospital", "Jail"].includes(curState) && isWs && v.status.state === "Okay" && (Date.now() - (current.status.updated || 0) < 60000)) {
  ...
  function watch() {
    ...
    member_lis.forEach((li, id) => {
      ...
      // Self-Healing: If visual is "Okay" but data is "Hospital" and expired -> sync
      if ((st.state === "Hospital" || st.state === "Jail") && nativeIsOK(status_DIV)) {
        const isWs = (st.description || "").includes("(WS)");
        // Check for valid timer (st.until is unix timestamp now)
      ...
      // Color logic
      if ((st.state === "Hospital" || st.state === "Jail") && !isExpired) safeSetAttr(status_DIV, COLOR, "red");
      else if (["Traveling", "Abroad"].includes(st.state)) safeSetAttr(status_DIV, COLOR, "blue");
      else safeSetAttr(status_DIV, COLOR, "green");
      ...
      switch (st.state) {
        ...
        case "Abroad": case "Traveling":
          ...
          if (desc.includes("Traveling to ")) {
            ...
            currentSortWeights.set(id, 5); // Generic Traveling (WS)
