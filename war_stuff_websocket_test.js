...
(async function () {
  ...
  console.log(
      `%c[TWSEO] Script Loaded (v5.1.0 JSON) | Debug: ${DEBUG} | Sort Okay Score: ${SORT_OKAY_BY_SCORE}`,
  ...
  function createWebSocketConnection() {
      ...
      socket.onmessage = (event) => {
          try {
              if (event.data === '{}') { socket.send('{}'); return; }
              ...
              if (data.push?.pub?.data?.message?.namespaces?.users?.actions) {
                  ...
                  // NEW: Listen for 'updateStatus' (JSON) instead of 'updateIcons'
                  if (actions.updateStatus) {
                      processWebSocketStatus(actions.updateStatus);
                  }
              }
          } catch (e) { console.error("[TWSEO] WS Error:", e); }
      };
