...
(async function () {
  ...
  console.log(
      `%c[TWSEO] Script Loaded (v5.3.0) | Debug: ${DEBUG}`,
  ...
  function processWebSocketStatus(updateData) {
      ...
      updates.forEach(u => {
          ...
          if (currentData.status.state !== newState || timerDiff > 5) {
              ...
              // --- STORE COLOR ON INPUT ---
              currentData.status.color = s.color || "green";
    ...
    member_lis.forEach((li, id) => {
      ...
      switch (st.state) {
        case "Fallen": case "Federal":
            ...
            safeSetAttr(status_DIV, COLOR, "red"); // old and redundant but safe to leave
