...
// @version     1.0.2
// @author      MrChurch [3654415] + Szanti
...
const Show = Object.freeze({
    ...
    RESP_UNAVAILABLE: "Can't show respect (FFScouter unavailable)"
});
...
(async function () {
    ...
    let ffscouter_api_key = GM_getValue("ffscouter-api-key", "");
  ...
  `);
    ...
    {
        try {
            GM_registerMenuCommand('Set Torn API Key', function setApiKey() {
                const new_key = prompt("Please enter a public Torn api key", api_key);
                if (new_key?.length === 16) {
                    GM_setValue("api-key", new_key);
                    api_key = new_key;
                    // Also reset FFScouter status if it was broken
                    can_ffscouter = true;
            ...
            if (api_key.charAt(0) === "#") {
                throw new Error("Please set the public Torn api key in the script manually on line 18.");
            }
        }

        try {
            GM_registerMenuCommand('Set FFScouter API Key', function setFfKey() {
                const new_key = prompt("Please enter your FFScouter API Key (leave empty to use your Torn API Key)", ffscouter_api_key);
                if (new_key !== null) {
                    GM_setValue("ffscouter-api-key", new_key);
                    ffscouter_api_key = new_key;
                    can_ffscouter = true;
                    // Trigger update
                    for (const row of document.querySelector(".tableWrapper > ul").childNodes) updateFf(row);
                }
            ...
            console.warn("Failed to register FFScouter Key menu command");
        }
    ...
    function mainLoop() {
        ...
        if (ff_updates.length > 0) {
            ...
            // Use specific FFScouter key if available, otherwise fallback to general key
            const key_to_use = ffscouter_api_key || api_key;

            GM.xmlHttpRequest({
                url: `https://ffscouter.com/api/v1/get-stats?key=${key_to_use}&targets=${scouts.map(getId).join(",")}`,
    ...
    function parseTable(table) {
        ...
        function reworkRow(row) {
            ...
            // FIX: use parentNode instead of looking for specific container class
            if (ff.nextElementSibling !== description) {
                const parent = ff.parentNode;
                // Ensure they share the same parent before moving
                if (parent && description.parentNode === parent) {
                    parent.insertBefore(description, ff);
                }
            }
            ...
            let last_timer = row.timer =
                setTimeout(function updateTimer() {
                    ...
                    if (time_left > 0 && last_timer === row.timer) {
                        ...
                        target.status = "Okay";
                        target.hospital = 0; // Reset hospital timer
                        setStatus(row);
                    }
                });
        }
